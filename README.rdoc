...
(There is no need to put the full path "users/1/profile", just translate each slug)

The helpers inside views and controllers will continue working the same but I18n.locale will be use as default locale:
  about_path # /en/about

This gem will not provide a default locale selection in the controller, since you may be interested in differente types of logics, you need to add it manually. Here it's a very simple example:
